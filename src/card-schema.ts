...
export function buildGenerationPrompt(input: GenerationPromptInput): string {
  ...
  return [
    ...
    "",
    "Before returning the two files, treat them as candidate skill artifacts and self-check: schema gate",
    "(valid JSON matching the contract), render gate (stress, thought groups, contours, and drill rounds",
    "will render without blank cards), fidelity gate (stay inside the learner brief), and practice-value",
    "gate (the lesson is immediately speakable and drillable). Do not invent unsupported domain facts.",
    "",
