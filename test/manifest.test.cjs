...
test("OpenAI-first UX and packaging metadata stay aligned", () => {
  ...
  assert.match(mediaSource, /function recorderBackend\(\) \{[\s\S]*backend === "macLocal"[\s\S]*backend === "webview"[\s\S]*backend === "auto"[\s\S]*: "macLocal"/);
  ...
  assert.match(practiceViewSource, /if \(payload\.type === "startNativeRecording" && this\.view === view\) \{\s*this\.pendingPriorTurn = undefined;\s*\}/);
  ...
  assert.match(mediaSource, /configureMaterials"\)\.addEventListener\("click", \(\) => \{[\s\S]*blockSetupChangeDuringPractice\(\)[\s\S]*postSidebarCommand\("configureMaterials"/);
  assert.match(mediaSource, /openTask"\)\.addEventListener\("click", \(\) => \{[\s\S]*blockIfPracticeTurnInProgress\("Finish or stop the current turn before opening the task card\."\)[\s\S]*postSidebarCommand\("openTask"/);
  assert.match(mediaSource, /openFolder"\)\.addEventListener\("click", \(\) => \{[\s\S]*blockIfPracticeTurnInProgress\("Finish or stop the current turn before opening the session folder\."\)[\s\S]*postSidebarCommand\("openSessionFolder"/);
  ...
  assert.match(mediaSource, /"refresh"\)\.addEventListener\("click", \(\) => \{[\s\S]*blockIfPracticeTurnInProgress\("Current turn is still running — wait for it to finish before refreshing\."\)[\s\S]*blockIfTransientActionInProgress\("refreshing"\)[\s\S]*beginRefreshRequest\(\$\("refresh"\)\)[\s\S]*type: "refresh"/);
  ...
  assert.match(mediaSource, /completeLocal"\)\.addEventListener\("click", \(\) => \{[\s\S]*blockIfPracticeTurnInProgress\("Finish or stop the current turn before completing this lesson\."\)[\s\S]*blockIfTransientActionInProgress\("completing this lesson"\)[\s\S]*type: "completeLocal", requestId/);
  ...
  assert.match(mediaSource, /message\.type === "error"[\s\S]*const errorText = messageErrorText\(message\.message, "Error\."\)[\s\S]*resetTransientActionBusyState\(errorText\)[\s\S]*renderMissingSourceSetup\(errorText\)[\s\S]*setStatus\(errorText, "error"\)/);
  ...
  assert.match(mediaSource, /normalizeProviderForSetting\("ttsProvider", settings\.ttsProvider \|\| "openai"\)/);
