...
test("runtime model strings are trimmed before UI state and provider calls", () => {
  for (const [source, pattern] of [
    ...
    [settingsSource, /qwenCoachModel: configString\("qwenCoachModel", "qwen3\.7-plus"\)/],
    ...
    [coachSource, /configString\("qwenCoachModel", "qwen3\.7-plus"\)/],
...
test("Qwen-first UX and packaging metadata stay aligned", () => {
  ...
  assert.match(coachSource, /fetchWithTimeout\(`\$\{baseUrl\}\/v1\/messages`/);
  ...
  assert.match(providerRoutesSource, /Configure Qwen Coach\/Analysis Key/);
  ...
  assert.match(coreSource, /qwenCoachSecretKey = "englishTraining\.qwenTokenPlanApiKey"/);
  assert.match(coreSource, /QWEN_TTS_BASE_HTTP_API_URL = "https:\/\/dashscope\.aliyuncs\.com\/api\/v1"/);
  assert.match(coreSource, /process\.env\.DASHSCOPE_API_KEY/);
  assert.match(coachSource, /getRequiredQwenCoachKey\(context\)/);
  assert.match(coachSource, /normalizedQwenCoachBaseUrl\(\)/);
  ...
  assert.match(settingsSource, /normalizedQwenCoachBaseUrl\(\)/);
  ...
  assert.match(ttsSource, /fetchWithTimeout\(qwenTtsGenerationUrl\(normalizedQwenTtsEndpoint\(\)\)/);
